...
class TaskDialog(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        # Title
        title_label = tk.Label(main_frame, text="📝 THÔNG TIN TASK", 
                              font=("Arial", 16, "bold"), fg="#4A90E2", bg='white')
        ...
        priority_combo.pack(fill=tk.X, pady=(5, 0))
        ...
        status_combo = ttk.Combobox(status_frame, textvariable=self.status_var,
                                   values=["Đang chờ", "Đang tiến hành", "Hoàn thành"], state="readonly")
        ...
        category_combo = ttk.Combobox(main_frame, textvariable=self.category_var,
                                     values=list(self.categories), state="readonly")
        category_combo.pack(fill=tk.X, pady=(5, 20))
        
        # Buttons
        btn_frame = tk.Frame(main_frame, bg='white')
        btn_frame.pack(fill=tk.X, pady=(20, 0))
        
        # Create button with callback to parent
        create_btn = tk.Button(btn_frame, text="✅ Tạo", command=self.create_task,
                              bg='#5CB85C', fg='white', font=("Arial", 11, "bold"),
                              padx=20, pady=8)
        create_btn.pack(side=tk.RIGHT, padx=(10, 0))
        
        save_btn = tk.Button(btn_frame, text="💾 Lưu", command=self.save_task,
                            bg='#5BC0DE', fg='white', font=("Arial", 11, "bold"),
                            padx=20, pady=8)
        save_btn.pack(side=tk.RIGHT, padx=(10, 0))
        
        cancel_btn = tk.Button(btn_frame, text="❌ Hủy", command=self.destroy,
...
class TaskManagerApp(tk.Frame):
    ...
    def initialize_user(self, email):
        ...
        sample_tasks = [
            Task("Hoàn thành báo cáo", "Viết báo cáo tháng 8", "Cao", "Đang tiến hành", "Work"),
            Task("Mua sắm cuối tuần", "Mua thực phẩm cho tuần", "Trung bình", "Đang chờ", "Shopping"),
            Task("Học Python", "Hoàn thành khóa học online", "Cao", "Đang tiến hành", "Learning")
        ]
        for task in sample_tasks:
            self.task_manager.add_task(task)
            if task.category in self.categories:
                self.categories[task.category]["tasks"].append(task.task_id)
...
Ngày tạo: {task.created_date}"""
    ...
    def update_status_display(self):
        total_tasks = len(self.task_manager.get_all_tasks()) if self.task_manager else 0
    ...
    def add_task(self):
        ...
        if dialog.result:
            if dialog.result.get('action') == 'create':
                # Create and immediately add task
                task = Task(
                ...
                # Just show the dialog was opened
                messagebox.showinfo("Thông báo", "Dialog đã được mở để nhập thông tin!")
    ...
    def apply_filters_and_search(self):
        ...
        if self.current_category == "All Tasks":
            tasks = [t for t in self.task_manager.get_all_tasks() if getattr(t, 'source', 'manual') == 'manual']
        else:
            assigned_task_ids = self.categories[self.current_category].get("tasks", [])
            tasks = [
